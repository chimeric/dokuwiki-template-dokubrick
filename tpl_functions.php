<?php
/**
 * DokuWiki Template DoKubrick Functions
 *
...
$sbl   = explode(',',tpl_getConf('sidebar_content'));
...
function tpl_sidebar() {
    ...
    $sb_order   = explode(',', tpl_getConf('sidebar_order'));
    $sb_content = explode(',', tpl_getConf('sidebar_content'));
 ...
 * @author Michael Klier <chi@chimeric.de>
 */
function tpl_sidebar_dispatch($sb) {
    global $lang;
    global $conf;
    global $ID;
    global $REV;
    ...
    switch($sb) {
        ...
        case 'toc':
            if(auth_quickaclcheck($svID) >= AUTH_READ) {
                ...
                $toc = preg_replace('/id="(.*?)"/', 'id="sb__\1"', Doku_Renderer_xhtml::render_TOC(p_get_metadata($svID,'description tableofcontents')));
                if(!empty($toc)) {
